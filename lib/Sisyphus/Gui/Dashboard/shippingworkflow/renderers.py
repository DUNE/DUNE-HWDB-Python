...
from .helpers import format_location_history_rows, generate_preshipping_csv, generate_preshipping_email_html, generate_receiving_email_html, generate_shipping_sheet, generate_shipping_email_html, generate_shipping_csv, get_country_options, get_institution_options, get_location_history_rows, get_preshipping_gate_status, get_shipping_service_type_from_hwdb, is_surf_route, route_label
...
def render_launcher(pid: str, record: dict, lan_mode: bool) -> str:
    ...
    location_history_rows = format_location_history_rows((part_info or {}).get('locations') or [])
    ...
    last_sync_text = escape(record.get('last_sync_time') or 'Initial sync only. No manual sync has been run yet.')
  ...
  <div class='page'>
    ...
    <section class='hero'>
      ...
      <div class='banner small'>This page is loaded from the saved workflow record and will not auto-sync to the HWDB. Use the button below only when you want a fresh refresh from the HWDB.</div>
      <div class='btnrow'>
        <div class='muted'>Last manual sync: {last_sync_text}</div>
        <form method='post' action='/shipping-workflow/sync' style='margin:0;'>
          <input type='hidden' name='pid' value='{escape(pid)}'/>
          <input type='hidden' name='k' value='{escape((request.args.get('k') or '').strip())}'/>
          <button type='submit' class='btn btn-next'>Sync to HWDB</button>
        </form>
      </div>
    </section>
...
def render_transit1(pid: str, record: dict, scene_index: int, error: str = '') -> str:
    ...
    history_rows = get_cached_location_history_rows(record, pid)
