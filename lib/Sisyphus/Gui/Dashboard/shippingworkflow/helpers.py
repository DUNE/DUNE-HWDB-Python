...
def format_location_history_rows(data: list[dict[str, Any]] | None) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    for eachloc in data or []:
...
def get_cached_location_history_rows(record: dict[str, Any] | None = None, pid: str | None = None) -> list[dict[str, str]]:
    ws = (record or {}).get("workflow_state") or {}
    cached = ws.get("LocationHistoryRows")
...
def cache_workflow_hwdb_snapshot(record: dict[str, Any], refresh: bool = True) -> dict[str, Any]:
    ws = record.setdefault("workflow_state", {})
    part_id = ((ws.get("part_info") or {}).get("part_id")) or record.get("pid")
    if not part_id:
        return record

    # part info + shipping checklist snapshot
    state = download_part_info(part_id, refresh=refresh)
    if isinstance(state, dict):
        if state.get("part_info") is not None:
            record["part_info"] = state.get("part_info") or {}
            ws["part_info"] = state.get("part_info") or {}
        for key in [
            "PreShipping1", "PreShipping2", "PreShipping3", "PreShipping4a", "PreShipping4b", "PreShipping5", "PreShipping6", "PreShipping7",
            "Shipping1", "Shipping2", "Shipping3", "Shipping4", "Shipping5", "Shipping6",
        ]:
            if key in state:
                ws[key] = state.get(key) or {}

    # cached location history
    try:
        resp = ra.get_hwitem_locations(part_id)
        ws["LocationHistoryRows"] = format_location_history_rows((resp or {}).get("data") or [])
    except Exception:
        logger.exception("Failed to cache location history for %s", part_id)
        ws.setdefault("LocationHistoryRows", [])

    # cached executive summary / qaqc gate
    try:
        item = dm.HWItem(part_id=part_id, refresh=refresh).data
        ...
        logger.exception("Failed to cache QA/QC gate status for %s", part_id)
        status_id = None
        status_name = ""
        certified = False
        uploaded = False
    ...
    try:
        ...
        logger.exception("Failed to cache Executive Summary image status for %s", part_id)
    ...
    # cached shipping service type
    try:
        shipping_service_type = "Domestic"
        hts_value = None
        specs = item.get("specifications") or []
        data_block = (specs[-1] or {}).get("DATA") or {} if specs else {}
        checklist_entries = data_block.get("Pre-Shipping Checklist")
        hts_value = _extract_checklist_value(checklist_entries, "HTS code")
        if hts_value is not None and str(hts_value).strip() not in {"", "None", "null"}:
            shipping_service_type = "International"
        ...
        logger.exception("Failed to cache shipping service type for %s", part_id)

    record["last_sync_time"] = datetime.now().isoformat(timespec="seconds")
    return record
...
def get_location_history_rows(part_id: str) -> list[dict[str, str]]:
    ...
    return format_location_history_rows(data)
...
def get_preshipping_gate_status(record: dict[str, Any] | None = None, pid: str | None = None) -> dict[str, Any]:
    ws = (record or {}).get("workflow_state") or {}
    cached = ws.get("PreShippingGateStatus")
    if isinstance(cached, dict):
        return cached

    part_id = pid or (ws.get("part_info") or {}).get("part_id") or ((record or {}).get("part_info") or {}).get("part_id")
    if not part_id:
        return {
            "status_id": None,
            "status_name": "",
    ...
    # Fallback only when no cached sync snapshot exists yet.
