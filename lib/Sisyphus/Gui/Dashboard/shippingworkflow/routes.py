...
from .helpers import cache_workflow_hwdb_snapshot, generate_receiving_email_html, get_institution_options, get_preshipping_gate_status, get_shipping_service_type_from_hwdb, patch_preshipping_to_hwdb, patch_shipping_to_hwdb, save_uploaded_artifact, update_receiving_location, update_shipping_location, update_transit_location
...
def _load_or_init_record(pid: str) -> dict:
    ...
    record = cache_workflow_hwdb_snapshot(record, refresh=True)
    save_workflow_progress(pid, record)
    return record


def _sync_record_from_hwdb(pid: str, record: dict | None = None) -> dict:
    record = deepcopy(record or _load_or_init_record(pid))
    record = cache_workflow_hwdb_snapshot(record, refresh=True)
    save_workflow_progress(pid, record)
    return record
...
def register_shipping_workflow_routes(app, lan_mode: bool = False) -> None:
    ...
    @server.post('/shipping-workflow/sync')
    def shipping_workflow_sync():
        pid = (request.form.get('pid') or '').strip()
        key = (request.form.get('k') or '').strip() or None
        if not pid:
            return redirect('/shipping-workflow')
        try:
            _sync_record_from_hwdb(pid)
        except Exception:
            logger.exception('Failed to sync shipping workflow for %s', pid)
