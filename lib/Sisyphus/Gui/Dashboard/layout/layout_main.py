...
from Sisyphus.Gui.Dashboard.utils.config import APP_VERSION, ENABLE_FDVD_TAB
...
from Sisyphus.Gui.Dashboard.layout.layout_fdvd              import fdvd_layout
...
def _dashboard_tabs():
    ...
    if ENABLE_FDVD_TAB:
        tabs.append(dcc.Tab(label="Hierarchy Chart", value="fdvd-tab", className="custom-tab"))
    return tabs


def _optional_fdvd_content():
    if not ENABLE_FDVD_TAB:
        return []
    return [
        html.Div(
            id="fdvd-tab-content",
            children=[fdvd_layout()],
            style={"display": "none"},
        )
    ]
...
def register_layout_callbacks(app):
    @app.callback(
        ...
        ] + ([Output("fdvd-tab-content", "style")] if ENABLE_FDVD_TAB else []),
        Input("tabs", "value"),
    )
    def switch_tabs(active_tab):
        ...
        if ENABLE_FDVD_TAB:
            visible_map["fdvd-tab"] = "fdvd-tab-content"
        return tuple(
